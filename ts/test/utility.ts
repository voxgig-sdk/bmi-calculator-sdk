...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.BMICALCULATOR_TEST_LIVE ||
    'TRUE' === process.env.BMICALCULATOR_TEST_OVERRIDE
  ) {
  ...
  m.BMICALCULATOR_TEST_EXPLAIN = process.env.BMICALCULATOR_TEST_EXPLAIN || m.BMICALCULATOR_TEST_EXPLAIN
