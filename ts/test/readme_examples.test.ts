...
//     (\`new BmiCalculatorSDK(...)\` / \`BmiCalculatorSDK.test(...)\` become
//     \`BmiCalculatorSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'BmiCalculatorSDK'
...
const TEST_SEED = {"entity":{"bmi":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new BmiCalculatorSDK(...)\` and any
// \`BmiCalculatorSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BmiCalculatorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BmiCalculatorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
