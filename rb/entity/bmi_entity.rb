...
class BmiEntity
  ...
  def make
    opts = @_entopts.dup
    BmiEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BmiCalculatorHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Bmi, Hash] the current Bmi data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BmiCalculatorHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Bmi fields)
  def match_get
  ...
  # Load a single Bmi.
  #
  # @param reqmatch [BmiLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Bmi.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Bmi, Hash] the loaded Bmi; raises BmiCalculatorError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BmiCalculatorHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
