...
class BmiDirectTest < Minitest::Test
  def test_direct_load_bmi
    setup = bmi_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-bmi", setup[:live] ? "live" : "unit")
    if _should_skip
...
def bmi_direct_setup(mockres)
  ...
  if live
    ...
    client = BmiCalculatorSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
