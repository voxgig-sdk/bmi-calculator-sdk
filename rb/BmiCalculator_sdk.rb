...
require_relative 'BmiCalculator_types'
...
class BmiCalculatorSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = BmiCalculatorHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = BmiCalculatorHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, BmiCalculatorFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    BmiCalculatorUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = BmiCalculatorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = BmiCalculatorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = BmiCalculatorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = BmiCalculatorHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = BmiCalculatorError.new(
        "graphql_error", "BmiCalculatorSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = BmiCalculatorSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
