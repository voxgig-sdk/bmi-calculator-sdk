...
class TestBmiEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Bmi(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _bmi_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "bmi." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BMI_CALCULATOR_TEST_BMI_ENTID JSON to run live")
        client = setup["client"]
...
def _bmi_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/bmi/BmiTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("BMI_CALCULATOR_TEST_BMI_ENTID"))
    ...
    if env.get("BMI_CALCULATOR_TEST_LIVE") == "TRUE":
        ...
        client = BmiCalculatorSDK(helpers.to_map(merged_opts))

    _live = env.get("BMI_CALCULATOR_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("BMI_CALCULATOR_TEST_EXPLAIN") == "TRUE",
        "live": _live,
