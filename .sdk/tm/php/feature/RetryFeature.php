...
// BmiCalculator SDK retry feature
...
class BmiCalculatorRetryFeature extends BmiCalculatorBaseFeature
{
    ...
    public function init(BmiCalculatorContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BmiCalculatorContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BmiCalculatorContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BmiCalculatorContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
