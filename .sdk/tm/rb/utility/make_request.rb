...
module BmiCalculatorUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BmiCalculatorResponse.new({})
    ...
    if fetch_err
      ...
      response = BmiCalculatorResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BmiCalculatorResponse.new(fetched)
    else
