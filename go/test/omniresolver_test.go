...
import (
	...
	omni "github.com/voxgig-sdk/bmi-calculator-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BmiCalculatorError); is {
				out := map[string]any{
					"name":    "BmiCalculatorError",
...
func omniCtx(arg any, client *sdk.BmiCalculatorSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BmiCalculatorSDK); is {
			client = live
