...
local BmiEntity = {}
BmiEntity.__index = BmiEntity


function BmiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BmiEntity)
  self._name = "bmi"
  self._client = client
...
function BmiEntity:get_name()
  return self._name
end


function BmiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BmiEntity.new(self._client, opts)
end
...
function BmiEntity:mark_deleted()
  self._deleted = true
end


function BmiEntity:deleted()
  return true == self._deleted
end


function BmiEntity:data_set(args)
  if args ~= nil then
...
function BmiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BmiEntity:match_set(args)
  if args ~= nil then
...
function BmiEntity:stream(action, args, callopts)
...
function BmiEntity:_run_op(ctx, post_done)
  local utility = self._utility
