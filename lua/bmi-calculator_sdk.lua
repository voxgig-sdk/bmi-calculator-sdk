...
local BmiCalculatorSDK = {}
BmiCalculatorSDK.__index = BmiCalculatorSDK
...
BmiCalculatorSDK._make_feature = _make_feature


function BmiCalculatorSDK.new(options)
  local self = setmetatable({}, BmiCalculatorSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function BmiCalculatorSDK:options_map()
...
function BmiCalculatorSDK:get_utility()
  return Utility.copy(self._utility)
end


function BmiCalculatorSDK:get_root_ctx()
  return self._rootctx
end


function BmiCalculatorSDK:prepare(fetchargs)
...
function BmiCalculatorSDK:direct(fetchargs)
...
-- by passing it.
function BmiCalculatorSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function BmiCalculatorSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "BmiCalculatorSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function BmiCalculatorSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = BmiCalculatorSDK.new(sdkopts)
  sdk.mode = "test"
