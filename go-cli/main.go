// Command BmiCalculator-cli is a CLI + REPL for the BmiCalculator SDK,
...
const prompt = "bmi-calculator"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "bmi"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BMI_CALCULATOR_APIKEY (api key), BMI_CALCULATOR_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BMI_CALCULATOR_APIKEY carries the API key and
	// BMI_CALCULATOR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BMI_CALCULATOR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BMI_CALCULATOR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBmiCalculatorSDK(opts)
