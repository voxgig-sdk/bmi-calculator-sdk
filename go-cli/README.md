...
boru-driven command-line client **and** interactive REPL for the BmiCalculator
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bmi-calculator-cli)
make build

# 2. See usage (words, entities, env vars)
./bmi-calculator-cli --help

# 3. Provide credentials once, via the environment
export BMI_CALCULATOR_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./bmi-calculator-cli load 1 bmi            # {id:1} shorthand
./bmi-calculator-cli load '{id:1}' bmi       # explicit match map

# 5. Override the API base URL for a single call
BMI_CALCULATOR_BASE=https://api.example.com ./bmi-calculator-cli load 1 bmi

# 6. No arguments -> interactive REPL
./bmi-calculator-cli
bmi-calculator> load 1 bmi
bmi-calculator> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bmi-calculator-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/bmi-calculator-cli load 1 bmi
   ```
...
```sh
./bmi-calculator-cli load 1 bmi          # scalar shorthand for {id:1}
./bmi-calculator-cli load '{id:1}' bmi     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `bmi-calculator>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `bmi`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/bmi-calculator-cli`. |
...
(`./bmi-calculator-cli <expr>`) and interactively (the REPL), and expressions compose the
