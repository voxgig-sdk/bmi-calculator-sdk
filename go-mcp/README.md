...
[MCP](https://modelcontextprotocol.io) server exposing the BmiCalculator SDK as
two agent tools — `bmi-calculator_list` and `bmi-calculator_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bmi-calculator-mcp)
make build

# 2. Provide credentials via the environment
export BMI_CALCULATOR_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user bmi-calculator \
  -- /absolute/path/to/bmi-calculator-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./bmi-calculator-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// bmi-calculator_list: first page of records
{ "entity": "bmi" }
{ "entity": "bmi", "query": { } }

// bmi-calculator_load: one record by id
{ "entity": "bmi", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bmi-calculator-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user bmi-calculator \
     -- "$PWD"/dist/*/bmi-calculator-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `bmi-calculator_list` and `bmi-calculator_load` tools now appear
   in new sessions. Ask the agent to *"list bmi using bmi-calculator"*
   and it calls `bmi-calculator_list` with `{"entity":"bmi"}`.
...
```sh
export BMI_CALCULATOR_APIKEY=sk_live_xxx            # API key
export BMI_CALCULATOR_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `bmi-calculator_list` tool
...
### Call the `bmi-calculator_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `bmi-calculator_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `bmi-calculator_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BMI_CALCULATOR_APIKEY` | API key sent with every request. |
| `BMI_CALCULATOR_BASE` | Optional override of the API base URL. |
...
```sh
./bmi-calculator-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"bmi-calculator_load","arguments":{"entity":"bmi","query":{"id":1}}}}'
```
