...
class BmiEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new BmiEntity($this->_client, $opts);
    }

    /**
     * @param Bmi|array $args Bmi data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = BmiCalculatorHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Bmi|array The current Bmi data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Bmi fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = BmiCalculatorHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Bmi fields).
     */
     ...
     * Load a single Bmi.
     *
     * @param BmiLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed BmiLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Bmi|array The loaded Bmi as an assoc-array at the
     *   SDK boundary; throws BmiCalculatorError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = BmiCalculatorHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
