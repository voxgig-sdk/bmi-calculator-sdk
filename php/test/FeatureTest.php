...
// BmiCalculator SDK feature test
//
...
class FtCtrl extends BmiCalculatorControl
{
...
class FtClient
{
    ...
    private BmiCalculatorUtility $utility;

    public function __construct(BmiCalculatorUtility $utility, array $options)
    {
    ...
    public function get_utility(): BmiCalculatorUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BmiCalculatorContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BmiCalculatorContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BmiCalculatorContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BmiCalculatorConfig::make_feature($name);
    ...
    private static function buildUrl(BmiCalculatorSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BmiCalculatorError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BmiCalculatorResponse($fetched);
        }
    ...
    private function fail(BmiCalculatorContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BmiCalculatorContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
