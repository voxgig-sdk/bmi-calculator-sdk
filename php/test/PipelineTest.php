...
// BmiCalculator SDK pipeline test
//
...
{
    private static function utility(): BmiCalculatorUtility
    {
        return new BmiCalculatorUtility();
    }
    ...
    private static function ctx(array $over = []): BmiCalculatorContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new BmiCalculatorContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new BmiCalculatorOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof BmiCalculatorError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new BmiCalculatorOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = BmiCalculatorMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new BmiCalculatorOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = BmiCalculatorMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new BmiCalculatorOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = BmiCalculatorMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BmiCalculatorSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = BmiCalculatorMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new BmiCalculatorResponse([]),
            'result' => new BmiCalculatorResult([]),
        ]);
        [, $err] = BmiCalculatorMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new BmiCalculatorSpec([]),
            'response' => null,
            'result' => new BmiCalculatorResult([]),
        ]);
        [, $err] = BmiCalculatorMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new BmiCalculatorSpec(['step' => 's']),
            'response' => new BmiCalculatorResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new BmiCalculatorResult([]),
        ]);
        [, $err] = BmiCalculatorMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new BmiCalculatorSpec(['step' => 's']),
            'response' => new BmiCalculatorResponse(self::resp(200, ['v' => 1])),
            'result' => new BmiCalculatorResult([]),
        ]);
        [, $err] = BmiCalculatorMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new BmiCalculatorSpec(['step' => 's']),
            'response' => new BmiCalculatorResponse(self::resp(200, ['v' => 2])),
            'result' => new BmiCalculatorResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        BmiCalculatorMakeResponse::call($ctx);
    ...
    {
        $preset = new BmiCalculatorResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new BmiCalculatorSpec([]),
            'response' => new BmiCalculatorResponse([]),
            'result' => new BmiCalculatorResult([]),
        ...
        [$got, $err] = BmiCalculatorMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new BmiCalculatorResult([])]);
        [, $err] = BmiCalculatorMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new BmiCalculatorSpec([]), 'result' => null]);
        [, $err] = BmiCalculatorMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BmiCalculatorSpec(['step' => 's']),
            'result' => new BmiCalculatorResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new BmiCalculatorOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BmiCalculatorMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BmiCalculatorSpec(['step' => 's']),
            'result' => new BmiCalculatorResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new BmiCalculatorOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BmiCalculatorMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BmiCalculatorResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new BmiCalculatorSpec([]),
            'result' => new BmiCalculatorResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = BmiCalculatorMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BmiCalculatorMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new BmiCalculatorError('boom', 'boom');
        $utility->fetcher = function (BmiCalculatorContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BmiCalculatorSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = BmiCalculatorMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (BmiCalculatorContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = BmiCalculatorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(BmiCalculatorResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        BmiCalculatorMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (BmiCalculatorContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BmiCalculatorSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = BmiCalculatorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new BmiCalculatorResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new BmiCalculatorSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = BmiCalculatorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = BmiCalculatorMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = BmiCalculatorMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new BmiCalculatorResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, BmiCalculatorDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new BmiCalculatorResult(['ok' => false])]);
        $this->expectException(BmiCalculatorError::class);
        BmiCalculatorDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new BmiCalculatorResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', BmiCalculatorMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        BmiCalculatorMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new BmiCalculatorBaseFeature();
        $b = new BmiCalculatorBaseFeature();
        BmiCalculatorFeatureAdd::call($ctx, $a);
        BmiCalculatorFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): BmiCalculatorBaseFeature
    {
        $f = new BmiCalculatorBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        BmiCalculatorFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): BmiCalculatorContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new BmiCalculatorSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BmiCalculatorPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        BmiCalculatorPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new BmiCalculatorResponse(['headers' => null]),
            'result' => new BmiCalculatorResult([]),
        ]);
        BmiCalculatorResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        BmiCalculatorResultBody::call($ctx);
